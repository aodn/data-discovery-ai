...
class LinkGroupingAgent:
    ...
    def group_links(self, links: List[Dict[str, str]]) -> List[Dict[str, str]]:
        """
        ...
        if not links or len(links) == 0:
            raise ValueError("No links provided for classification")
        for link in links:
            ...
            if "href" not in keys or "title" not in keys:
                ...
                link_group = self.make_decision(link)
                if link_group:
                    link["group"] = link_group
        return links

    def make_decision(self, link: Dict[str, str]) -> Dict[str, str]:
        """
        ...
        for group, conditions in self.grouping_rules.items():
            ...
            if "title" in conditions:
                if group == "Document" and any(
                    keyword in title.lower() for keyword in conditions["title"]
                ):
                    return group
                elif group == "Data Access" and all(
                    any(word in title.lower() for word in group)
                    for group in conditions["title"]
                ):
                    return group
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if any(keyword in content for keyword in self.page_content_keywords):
                    return "Data Access"
        except:
