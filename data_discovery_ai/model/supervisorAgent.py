...
class SupervisorAgent(BaseAgent):
    def __init__(self):
        ...
        self.model_name_class_map = {
            "description formatting": DescriptionFormattingAgent
    ...
    def make_decision(self, request: Dict):
        ...
        if is_valid:
            ...
            for selected_model in selected_models:
                ...
                if AgentClass:
                    self.task_agents.append(AgentClass())
    ...
    def take_action(self, request: Dict):
        ...
        with Pool(processes=MAX_PROCESS) as pool:
            results = pool.map(self.run_agent, [(agent, request) for agent in self.task_agents])
    ...
    def is_valid_request(self, request: Dict[str, Union[str, list]]) -> bool:
        ...
        if not selected_models or not isinstance(selected_models, list):
            logger.error("No selected model found or format is invalid (expected a list).")
            return False

        for model_name in selected_models:
            model_config = next((m for m in AVAILABLE_AI_MODELS if m["model"] == model_name), None)
            if not model_config:
                logger.error(f"Invalid model name: {model_name}. Choose from {[m['model'] for m in AVAILABLE_AI_MODELS]}")
                return False

            required_fields = model_config["required fields"]
            for field in required_fields:
                if field not in request:
                    logger.error(f"Model '{model_name}' is missing required field: '{field}'")
                    return False
