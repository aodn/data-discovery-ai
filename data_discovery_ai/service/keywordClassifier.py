...
def keywordClassifier():
    ...
    threshold = 10
    rare_keyword = keyword_distribution_df[
        keyword_distribution_df["count"] <= threshold
    ]
    ...
    X_oversampled, Y_oversampled = preprocessor.resampling(
        X_train=X, Y_train=Y, strategy="custom", rare_keyword_index=rare_keyword_index
    )

    # # split train and test set
    # X_oversampled, Y_oversampled = X, Y
    dimension, n_labels, X_train, Y_train, X_test, Y_test = (
        preprocessor.prepare_train_validation_test(X_oversampled, Y_oversampled)
    )
    ...
    target_predicted_labels = model.prediction(
        target_X, trained_model, confidence, top_N
    )
