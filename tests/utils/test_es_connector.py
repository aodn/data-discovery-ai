...
class TestESConnector(unittest.TestCase):
    ...
    @patch("data_discovery_ai.utils.es_connector.Elasticsearch", side_effect=Exception("Connection Error"))
    ...
    def test_search_es_success(self, _):
        ...
        mock_client.search.side_effect = [
            {
                "hits": {"hits": [{"_source": {"val": 1}, "sort": [1]}]},
                "pit_id": "123"
            },
            {
                "hits": {"hits": [{"_source": {"val": 2}, "sort": [2]}]},
                "pit_id": "123"
            }
        ]
